...
def listAllFilesPath(path,formats=['png','jpg','tif','jpeg']):
    results=[]
...
#inter_cubic enlarges the image properly
...
def prepareData(size):
    ...
    for _class in range(num_classes):
        ...
        samples=np.array(list(map(preProcessImage,listAllFilesPath(path),repeat(size[0],length),repeat(size[1],length))))
    ...
    #shuffle the inputs and outputs the same away
...
#    model.compile(loss='cat')
    ...
    history=model.fit(x_train,y_train,batch_size=16,epochs=no_of_epochs,validation_data=(x_test,y_test))
